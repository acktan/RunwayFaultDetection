...
def test_get_edges_photos():
    ...
    min_lat, max_lat, min_lon, max_lon = extraction_airports_class.get_edges_photos(
        poly_boundaries
    )
...
def test_get_edges_intersection():
    ...
    (
        ...
        min_2,
        poly_crop,
    ) = extraction_airports_class.get_edges_intersection(
...
def test_get_cropping_coordinates():
    ...
    (
        ...
        min_2,
        max_2,
    ) = extraction_airports_class.get_cropping_coordinates(
        poly_intersection, poly_boundaries
    )
    assert up_poly_crop == [(0, 0), (180114, 0), (180114, 200641),
                            (0, 200641), (0, 0)]
...
def test_get_photos_edges_multi():
    ...
    (
        ...
        min_lon,
        max_lon,
    ) = extraction_airports_class.get_edges_photos_multi(poly_boundaries)
...
def test_extract_airports():
    ...
    path_output = (
        ...
        + conf["paths"]["airportextraction_file"]
    )
    output = extraction_airports_class.extract_airports(
        df, path_input, path_output
    )
...
def test_extract_airports_multi():
    ...
    """
    img_shape = (25000, 25000)
    extraction_airports_class = Extractionairports(
    conf, img_shape
    )
    _, df_multi_photos, dup_aero = extraction_airports_class.read_create_input()
    dup_aero = dup_aero[0]
    path_input = conf["paths"]["bdortho_input_path"]
    path_output = conf["paths"]["Outputs_path"] + conf["paths"][
    "airportextraction_file"
    ]
    output = extraction_airports_class.extract_airports_multi(
    df_multi_photos, dup_aero, path_input, path_output
    )
    assert output == None
'''
