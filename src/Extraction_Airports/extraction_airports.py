...
class Extractionairports:
    ...
    def read_create_input(self):
        ...
        Returns:
           ...
           df_multi_photos: A dataframe intersecting the
               BD_ORTHO and BD_CARTA data,
        ...
        # Create column for geometry of orthophotos and aeroports
        # to be used later
        ...
        df["geometry_intersection"] = df["geometry_aero"].intersection(
            df["geometry_photos"]
        )

        # remove lines with no TOPONYME
        dup_aero = df[df["TOPONYME"].duplicated()]["TOPONYME"].unique()
        df_multi_photos = df[df["TOPONYME"].isin(dup_aero)].reset_index(
            drop=True
        )
    ...
    def get_edges_photos(self, poly_boundaries):
        ...
        poly_bound = pd.DataFrame(
            [
                {"lat_bound": int(coords[1]), "lon_bound": int(coords[0])}
    ...
    def get_edges_intersection(self, poly_intersection, poly_boundaries):
        ...
        min_lat, max_lat, min_lon, max_lon = self.get_edges_photos(
            poly_boundaries
        )
        poly_mapped = mapping(poly_intersection)
        poly_coordinates = poly_mapped["coordinates"][0]

        # update coordinates of intersection from the map to the
        # picture reference
        poly_int = pd.DataFrame(
            [
                {
                    "nor_lat_int": self.normalize_val(
                        int(coords[1]), min_lat, max_lat, self.img_shape[0]
                    ),
                    "nor_lon_int": self.normalize_val(
                        int(coords[0]), min_lon, max_lon, self.img_shape[0]
                    ),
                }
        ...
        max_1, max_2 = poly_int.max()["nor_lat_int"], \
            poly_int.max()["nor_lon_int"]
        min_1, min_2 = poly_int.min()["nor_lat_int"], \
            poly_int.min()["nor_lon_int"]
        ...
        if poly_intersection.geom_type == "Polygon":
            ...
            for j in range(len(lst_multi)):
                ...
                (
                    ...
                    min_2_k,
                    poly_crop,
                ) = self.get_edges_intersection(poly_intersection,
                                                poly_boundaries)
                ...
                up_poly_crop_int = [
                    (
                        self.normalize_val(
                            coords[1], min_2_k, max_2_k, (max_2_k - min_2_k)
                        ),
                        self.normalize_val(
                            (self.img_shape[0] - coords[0]),
                            f_min_1_k,
    ...
    def get_edges_photos_multi(self, poly_boundaries):
        ...
        if poly_boundaries.geom_type == "Polygon":
            ...
            for j in range(len(poly_coordinates_bound)):
                ...
                # Get the top, bottom, right and left limits of
                # x and y for normaliz.
                min_lat_l.append(
                    min([poly_use[c][1] for c in range(len(poly_use))])
                )
                max_lat_l.append(
                    max([poly_use[c][1] for c in range(len(poly_use))])
                )
                min_lon_l.append(
                    min([poly_use[c][0] for c in range(len(poly_use))])
                )
                max_lon_l.append(
                    max([poly_use[c][0] for c in range(len(poly_use))])
                )
    ...
    def crop_save(self, image_adj, poly_crop_adj,
        ...
        if not multi:
            ...
            x, y, w, h = rect
            image_adj = image_adj[y: y + h, : x + w].copy()
        # (2) make mask
        pts = pts - pts.min(axis=0)
        mask = np.zeros(image_adj.shape[:2], np.uint8)
        cv2.drawContours(mask, [pts], -1,
                         (255, 255, 255), -1, cv2.LINE_AA)
        # (4) add the white background
        bg = np.ones_like(image_adj, np.uint8) * 255
        cv2.bitwise_not(bg, bg, mask=mask)
        bg = bg + cv2.bitwise_and(image_adj,
                                  image_adj, mask=mask)
        # save the result
        cv2.imwrite(path + "/cropped_cv2_" + path_im[:-4] +
                    "_" + q + ".jpg", bg)
    ...
    def extract_airports_multi(self, df, dup_aero, path_input, path_output):
        ...
        for i in range(len(dup_aero)):
            logger.info("Iteration " + str(i + 1) +
                        " out of " + str(len(dup_aero)))
            df_aero_i = df[
                df["TOPONYME"] == dup_aero[i]
                ...
                if df_aero_i.iloc[k][
                    ...
                    with rasterio.open(path_input + path_im) as src:
                        im_crop = src.read(
                            window=Window(
                                min_2, f_min_1,
                                (max_2 - min_2), (f_max_1 - f_min_1)
                            )
                        )
                        im_crop = np.swapaxes(im_crop, 0, 2)
                        im_crop = np.swapaxes(im_crop, 0, 1)
                    logger.info("Cropping photo...")
                    poly_mapped_int_coords = mapping(
                        df_aero_i.iloc[k]["geometry_photos"]
                    )
                    poly_mapped_int_coords = poly_mapped_int_coords[
                        "coordinates"
                    ][0]
                    lat_new_orig = (
                        (
                            min(poly_mapped_int_coords)[0]
                            + (min_2 * 5000 / 25000)
                            - min_mi_lat
                        )
                        * 25000
                    ...
                    if k == 0:
                        ...
                        poly_last = poly_last.union(
                            df_aero_i.iloc[k]["geometry_intersection"]
                        )
                    img_fin[
                        int(lon_new_orig): int(
                            (lon_new_orig + f_max_1 - f_min_1)
                        ),
                        int(lat_new_orig): int(lat_new_orig + max_2 - min_2),
                        :,
                    ] = im_crop[:, :, :]
                    # UNION OF POLYGONS
                else:
                    ...
                    for q in range(len(up_poly_crop)):
                        ...
                        lon_new_orig = (
                            (
                                max_ma_lon
                                - max(poly_mapped_int_coords)[1]
                                + (f_min_1[q] * 5000 / 25000)
                            )
                            * 25000
                            / 5000
                        )
                        if (k == 0) and (q == 0):
                            poly_last = df_aero_i.iloc[k][
                                "geometry_intersection"
                            ]
                        else:
                            poly_last = poly_last.union(
            ...
            for p in range(len(map_last)):
                if p == 0:
                    pop_lst = [
                        (pp[0], min_mi_lat, height, pp[1], max_ma_lon, width)
    ...
    def extract_all_airports(self):
        ...
        path_output = (
            self.conf_path["Outputs_path"]
            + self.conf_path["folder_extraction_airports"]
        )
        path_input = self.conf_path["bdortho_input_path"]
        self.extract_airports(df, path_input, path_output)
        self.extract_airports_multi(df_multi_photos, dup_aero,
