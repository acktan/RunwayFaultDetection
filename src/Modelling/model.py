...
class Model:
    ...
    def macro_soft_f1(self, y, y_hat):
        """Compute the macro soft F1-score as a cost (average 1 - soft-F1
            across all labels).
        Use probability values instead of binary predictions.

        Args:
            y (int32 Tensor): targets array of shape (BATCH_SIZE, N_LABELS)
            y_hat (float32 Tensor): probability matrix from forward
                propagation of shape (BATCH_SIZE, N_LABELS)
    ...
    def macro_f1(self, y, y_hat):
        """Compute the macro F1-score on a batch of observations (average F1
            across labels)
        ...
        fp = tf.cast(tf.math.count_nonzero(y_pred * (1 - y), axis=0),
                     tf.float32)
        fn = tf.cast(tf.math.count_nonzero((1 - y_pred) * y, axis=0),
                     tf.float32)
    ...
    def load_my_model(self):
        ...
        Args:
            ...
            custom_objects (dict): dictionnary of the custom object
                integrated into the model

        Returns:
            loaded_model (keras.engine.sequential.Sequential)
        """
        path = self.conf["paths"]["Outputs_path"] + \
            self.conf["paths"]["folder_model"]
        ...
        loaded_model = keras.models.load_model(path + model, custom_objects)
